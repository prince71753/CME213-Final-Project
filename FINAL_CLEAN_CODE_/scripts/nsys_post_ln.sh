...
#SBATCH -p gpu-turing
#SBATCH --gres=gpu:1
#SBATCH -o logs/nsys_post_ln_%j.out
#SBATCH -e logs/nsys_post_ln_%j.err
...
run_nsys() {
    ...
    local rep="results/nsys_post_ln_${JOB}_${label}"
    echo "=== ${label} ==="
    cme213_clean_env CME213_GEMM_BACKEND="$backend" $env_var \
        ...
        > "logs/nsys_post_ln_${JOB}_${label}.txt" 2>&1 || true
    nsys stats --report cuda_gpu_kern_sum -f csv "${rep}.nsys-rep" \
        > "results/nsys_post_ln_${JOB}_${label}_kern.csv" 2>/dev/null || true
    echo "--- top kernels ---"
    head -n 25 "results/nsys_post_ln_${JOB}_${label}_kern.csv" || true
}
...
run_nsys "h128_cublas_tc_nograph" build_h128 cublas_tc nograph 200 1e-4

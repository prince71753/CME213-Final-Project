// GEMM backend correctness and timing test.
#include "common.h"
#include "kernels.h"
#include <cstdio>
#include <cstdlib>
...
int main() {
    ...
    const char* requested_backend = gemm_requested_backend_name();
    const char* strict_env = getenv("CME213_STRICT_FP32");
    bool strict_fp32 = strict_env && strcmp(strict_env, "1") == 0;
    bool custom_backend = strcmp(requested_backend, "custom") == 0;
    float tiled_tolerance = custom_backend ? 1e-5f
                                           : (strict_fp32 ? 1e-3f : 5e-3f);
    printf("GEMM requested backend: %s\n", requested_backend);
    printf("GEMM auto policy: %s\n", gemm_auto_policy_name());
    printf("CME213_STRICT_FP32: %s\n", strict_fp32 ? "1" : "0");
    printf("gemm_tiled tolerance: %.1e\n\n", tiled_tolerance);
    ...
    for (auto& sz : sizes) {
        ...
        bool naive_pass = err_naive < 1e-3f;
        all_pass = all_pass && naive_pass;
        printf("  naive  max error: %.6e  %s\n", err_naive, naive_pass ? "PASS" : "FAIL");
        ...
        bool tiled_pass = err_tiled < tiled_tolerance;
        all_pass = all_pass && tiled_pass;
        printf("  gemm_tiled(dispatch) max error: %.6e  %s\n",
               err_tiled, tiled_pass ? "PASS" : "FAIL");

...
- `results/`: csv and md result artifacts used for tables.
- `logs/`: selected validation, benchmark, and profiling logs.
- `profiles/`: selected Nsight profile summaries and exported reports.
...
result is the h256 openmp communication-thread path, which improves over
blocking synchronization when enough backward work exists to hide communication.
the report uses `results/REAL_RESULTS.md` and the top-level
`../FINAL_TRUTH_TABLE.md` as the current source of truth.  the h512 ffn fp16
path is kept only as an opt-in research result because it passed focused
reference checks but was not fully valid in repeated h512 training.

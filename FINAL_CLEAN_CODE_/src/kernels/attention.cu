// CUDA attention forward and backward kernels for the transformer block.
#include "kernels.h"
#include <cfloat>
...
__global__ void compute_ds_kernel(const float* P, const float* dP,
    ...
    float partial = 0.0f;
    for (int d = threadIdx.x; d < hd; d += blockDim.x)
        partial += dO[base + row * hd + d] * O[base + row * hd + d];
    ...
    if (blockDim.x <= 32) {
        if (threadIdx.x == 0) s_dot = partial;
    } else {
        __shared__ float s_warp[32];
        int lane    = threadIdx.x & 31;
        int warp_id = threadIdx.x >> 5;
        int nwarps  = (blockDim.x + 31) >> 5;
        if (lane == 0) s_warp[warp_id] = partial;
        __syncthreads();
        if (warp_id == 0) {
            float v = (threadIdx.x < nwarps) ? s_warp[threadIdx.x] : 0.0f;
            for (int offset = 16; offset > 0; offset >>= 1)
                v += __shfl_xor_sync(0xffffffff, v, offset);
            if (threadIdx.x == 0) s_dot = v;
        }
    }
    __syncthreads();
    float dot_oo = s_dot;

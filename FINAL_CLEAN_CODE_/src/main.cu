// Training driver, command-line parsing, validation output, and profiling hooks.
#include "common.h"
#include "data.h"
#include "model.h"
...
static ParamStats param_stats(const TransformerModel& model) {
    std::vector<float> h(model.total_param_count);
    CUDA_CHECK(cudaMemcpy(h.data(), model.d_all_params,
                          model.total_param_count * sizeof(float),
                          cudaMemcpyDeviceToHost));
    ...
    for (float v : h) {
        ...
        stats.sum += dv;
        stats.sumsq += dv * dv;
        stats.max_abs = std::max(stats.max_abs, std::fabs(dv));

        uint32_t bits = 0;
        std::memcpy(&bits, &v, sizeof(bits));
        stats.hash ^= (unsigned long long)bits;
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (strcmp(argv[i], "--trace-trajectory") == 0) {
    ...
    if (dist.is_root()) {
        ...
        printf("Training: epochs=%d lr=%.2e max_steps=%d world_size=%d sync_mode=%s effective_sync=%s bucket_kb=%d auto_overlap_max_mb=%.1f\n",
               ...
               auto_overlap_max_mb);
        if (trace_trajectory) {
            printf("Trajectory trace: enabled, reporting loss and parameter statistics after each step\n");
        }
#ifdef USE_MPI
    ...
    for (int epoch = 0; epoch < num_epochs; ++epoch) {
        ...
        for (int step = 0; step < steps_per_epoch; ++step) {
            ...
            if (trace_trajectory) {
                ...
                ParamStats stats = param_stats(model);
                double sum_min = dist.min_double(stats.sum);
                double sum_max = dist.max_double(stats.sum);
                double sumsq_min = dist.min_double(stats.sumsq);
                double sumsq_max = dist.max_double(stats.sumsq);
                double maxabs_min = dist.min_double(stats.max_abs);
                double maxabs_max = dist.max_double(stats.max_abs);
                if (dist.is_root()) {
                    step_loss = model.read_loss();
                    printf("TRAJECTORY epoch=%d step=%d loss=%.9g "
                           "param_sum=%.17g param_sumsq=%.17g "
                           "param_maxabs=%.9g param_hash=%016llx "
                           "sum_span=%.3e sumsq_span=%.3e maxabs_span=%.3e\n",
                           epoch + 1, step + 1, step_loss,
                           stats.sum, stats.sumsq, stats.max_abs,
                           stats.hash,
                           sum_max - sum_min, sumsq_max - sumsq_min,
                           maxabs_max - maxabs_min);
                }
            }
